...
export class LoggedUser extends User {
	...
	async get_corrector_slots(): Promise<CorrectionSlot[]> {
		...
		const objs: CorrectionSlot[] = res.map(
			(slot: any) => new CorrectionSlot(slot)
		);
	...
	async get_slots_for_project(
		project: Project | number
	): Promise<CorrectionSlot[]> {
		const id = typeof project == "number" ? project : project.id;
		const res = await this.fetch(
			`/projects/${id}/slots?filter[future]=true`
		);
		const objs: CorrectionSlot[] = res.map(
			(slot: any) => new CorrectionSlot(slot)
		);
